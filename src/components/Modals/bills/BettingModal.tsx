...
import { bettingTransferSchema, BettingTransferType } from '@/lib/validation'
...
const BettingModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<BettingTransferType>({
    resolver: zodResolver(bettingTransferSchema),
  })
  ...
  const processForm: SubmitHandler<BettingTransferType> = (data) => {
    console.log(data)
    try {
      Swal.fire({
  ...
  type FieldName = keyof BettingTransferType
  ...
  const handleProviderSelect = (bettingProvider: string) => {
    setValue('provider', bettingProvider)
        ...
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Betting</h5>
              <button
                type="button"
                className="btn-close"
                aria-label="Close"
                            ...
                            <CommandGroup>
                              {BettingProviders.map((item) => (
                                <CommandItem key={item.name}>
                                  <p
                                    onClick={() =>
                                      handleProviderSelect(item.name)
                                    }
              ...
              >
                ...
                <div className="tw-flex tw-flex-col tw-gap-2 tw-text-xl">
                  <div className="tw-flex tw-justify-between tw-items-center">
                    <p>User ID:</p>
                    <p className="tw-font-bold tw-truncate">{userIdValue}</p>
                  </div>
                </div>
                <div className="tw-flex tw-flex-col tw-gap-2 tw-text-xl">
                  <div className="tw-flex tw-justify-between tw-items-center">
